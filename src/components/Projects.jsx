...
const Projects = ({ location }) => {
  ...
  return (
    <>
      <ul className="mt-40 flex flex-col gap-y-10 sm:mt-12 sm:flex-row sm:gap-x-2.5 sm:gap-y-0">
        {projects.map(({ id, client, img, service }) => (
          <li key={id} className="w-full sm:w-[54.5rem]">
            <button
              className="border-0"
              onClick={() => handleClick(label, description, projectsItm, service)}
            >
              <img src={img} alt={client} className="w-full rounded-md" />
